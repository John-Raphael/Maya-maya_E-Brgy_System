...
class FormHandler extends mysqli
{
    ...
    protected function min($minLength)
    {
        ...
        if (strlen($value) < $minLength) {
            response(false, "$alias should be {$minLength} in length.");
        }
    }
    ...
    public function reCAPTCHA(){
        $reCAPTCHA_enabled = $GLOBALS["reCAPTCHA_enabled"];
        $this->reCAPTCHA = $reCAPTCHA_enabled;
        return $this;
    }
    public function status(){
        $reCAPTCHA_enabled = $GLOBALS["reCAPTCHA_enabled"];
        $reCAPTCHA_secret_key = $GLOBALS["reCAPTCHA_secret_key"];
...
class NoSql
{
    ...
    {
        $this->DB_SELECTED_COLUMNS = null;
        $this->DB_WHERE = null;
        $this->DB_JOINS_STRUCTURE = [];
        $this->DB_SORT_ORDER = null;
        $this->DB_LIMIT = null;
...
class Session
{
    ...
    {
        return isset($_SESSION[$this->_session_key]);
    }
...
class Server{
    public function ValidateRequest(string $key = "")
    {
        if ($key == "") {
            response(false,"Please enter the csrf key");
        }
        $key = $key == "" ? "" : "_$key";
